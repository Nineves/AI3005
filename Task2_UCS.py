...
class UCS:
    ...
    def AStarSearchProcess(self, start, end):
        ...
        srcNode.setBudget(0)
        self.visited[start] = srcNode.distFromSrc
        ...
        for node in self.G[start]:
            ...
            FCost = newNode.distFromSrc
            self.pq.put((FCost, newNode))
        # print(pq)

        while not self.pq.empty():
            newNode = self.pq.get()[1]
            if (newNode.name in self.visited and newNode.distFromSrc >= self.visited[newNode.name] and newNode.budget >=
                    visitedBudget[newNode.name]):
                continue
            curPath.append(newNode)
            if newNode.name in self.visited:
                if newNode.distFromSrc < self.visited[newNode.name]:
                    self.visited[newNode.name] = newNode.distFromSrc
            ...
            for adjNode in self.G[newNode.name]:
                ...
                self.pq.put((DValue, adjNewNode))
        return curPath

    def UCSbegin(self, start, end):
